...
function onEdit(e) {
  ...
  const invalidSheet = ['テンプレート'];
  if (invalidSheet.includes(sheetName)) { return };

  const sheet = e.source.getActiveSheet();
  const editedCell = sheet.getActiveCell();
  const row = e.range.getRow();
  const col = e.range.getColumn();
  const columnName = convert_fromNum_toAlpha(col)
  ...
  let setting_title = ["変更前の値", "変更後の値", "アカウント名", "最終更新日"]
