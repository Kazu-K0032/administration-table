...
function changeAverage() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAMES.MONTHLY_TASK_LIST)
  if (!sheet.getName() == SHEET_NAMES.MONTHLY_TASK_LIST) return

  const titleValues = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues().flat()
  const judgeSrcColumn = CONSTANTS.PROGRESS_COLUMNS
  ...
  avr = avr.toFixed(2)
  console.log(sum, avr)
  sheet.getRange(1, CONSTANTS.CONTENT_COLUMN).setValue(`内容（${avr}％）`)
}
